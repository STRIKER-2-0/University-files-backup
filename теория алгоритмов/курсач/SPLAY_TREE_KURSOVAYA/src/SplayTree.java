...
public class SplayTree {
	...
	public void printAsTree() {			//печать как дерево, по уровням
		SplayTree Null=new SplayTree();		//вместо налла, метка конца уровня
		SplayTree Space=new SplayTree(Integer.MIN_VALUE);
		
		Queue<SplayTree> q= new ArrayDeque<>();
		int tabs=0;
        for(int i=0; i<depth(this,0); i++)		//посчет кол-ва табуляций
            tabs=2*tabs+1;
        print_symbol(tabs);
        q.add(this);
	...
	private int depth(SplayTree tr, int dep_count){         //глубина дерева
